    ...
    try:
        ...
        for log in recent_logs:
            recent_activity.append({
                ...
                "time": timezone.localtime(log.execution_time).strftime("%I:%M %p"),
                "full_logs": log.logs,
...
class TriggerSchedulesView(APIView):
    authentication_classes = []
    permission_classes = [AllowAny]

    def post(self, request):
        # 1. Convert strict UTC into your local timezone (Asia/Colombo)
        ...
        for task in tasks:
            # 2. Prevent Double-Firing: If it already successfully ran today, skip it
            if task.last_run and timezone.localtime(task.last_run).date() == current_date:
                continue

            task_time = task.execution_time.replace(second=0, microsecond=0)

            # 3. Resilient Time Check: Is it due or slightly past due?
                ...
                if should_run:
                    ...
                    try:
                        ...
                        TaskExecutionLog.objects.create(
                            ...
                            controller_name=task.assignment.controller.name,
                            status=status_text,
                    ...
                    # 4. Mark as run and deactivate if it's a one-time schedule
                    task.last_run = now
                    if task.recurrence == 'once':
                        task.is_active = False
                    task.save()

        return Response({"status": "success", "triggered_tasks": triggered_count})
