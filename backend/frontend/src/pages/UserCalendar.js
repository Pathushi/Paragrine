...
import {
  Clock, Calendar as CalIcon, Plus, X, RefreshCw, Zap, Trash2, Edit3, CheckCircle2, Activity,
} from "lucide-react";

const UserCalendar = () => {
  const [date, setDate] = useState(new Date());
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedLog, setSelectedLog] = useState(null); // <-- NEW: State for log viewer modal
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    try {
      if (isEditMode && selectedTaskId) {
        ...
        showNotification("Task added to queue");
      }
  ...
  // NEW: Proper Date matching using ISO string from backend
  ...
  return (
    <div className="p-8 fade-in font-sans relative">
      {notification.show && (
        <div className="fixed top-10 right-10 z-[100] bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3">
            ...
            <div className="space-y-2.5 overflow-y-auto flex-1 max-h-[180px]">
              {filteredActivities.length > 0 ? (
                filteredActivities.map((log) => (
                  <div 
                    key={log.id} 
                    onClick={() => setSelectedLog(log)} // <-- NEW: Makes the item clickable
      ...
      {/* RAW LOG VIEWER MODAL - Pops up when you click an executed log */}
      {selectedLog && (
