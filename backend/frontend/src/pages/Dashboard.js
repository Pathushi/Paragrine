...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const logoutUser = () => {
      ...
      alert("System has timed out due to inactivity. Please log in again.");
      window.location.href = "/";
    };
    const resetTimer = () => {
      clearTimeout(timeout);
      // Set for 1 hour (3,600,000 milliseconds)
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      loadProfileTelemetry();
      alert("System Profile configuration synchronized completely!");
    } catch (err) {
      console.error(err);
      alert("Operational issue executing identity updates.");
    }
  };
  ...
  const renderOverview = () => {
    // Basic search filtering for the recent activity log on the overview dashboard
  ...
  const renderUserOverview = () => {
    // Basic filtering for the user's workspace history
  ...
  const renderView = () => {
    // 1. Pass the search query down to the child views!
        ...
        <header className="flex items-center justify-between theme-card border border-slate-200/60 shadow-sm px-4 md:px-6 py-3 md:rounded-[2rem] rounded-2xl mb-4 md:mb-6 shrink-0 transition-all hover:shadow-md">
          ...
          <div className="relative w-full max-w-md hidden sm:block ml-4 md:ml-0">
            ...
            <input
              ...
              placeholder="Search..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)} // 2. Link state to the input field!
