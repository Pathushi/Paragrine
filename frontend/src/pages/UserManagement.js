...
const UserManagement = () => {
  ...
  const handleToggleAccess = async (user) => {
    ...
    try {
      ...
      loadUsers();
      toast.success(`Access ${user.is_enabled ? "revoked" : "granted"} for ${user.username}`);
    } catch (err) {
      console.error("Toggle failed", err);
      toast.error("System Error: Failed to update user status.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      loadUsers();
      toast.success(isEditMode ? "Account modified successfully" : "New user created successfully");
    } catch (err) {
